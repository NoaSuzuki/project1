...
Model* Model::LordFromOBJ(const std::string& modelname)
{
	Model* model = new Model();
	//デスクリプタヒープの生成
...
{
	HRESULT result;
	result = sDevice->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE, &CD3DX12_RESOURCE_DESC::Buffer((sizeof(ConstBufferDataB1) + 0xff) & ~0xff),
