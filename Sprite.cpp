...
#pragma endregion
#pragma region スプライト用のテクスチャ読み込み
...
#pragma endregion
#pragma region 描画後の処理
//描画後処理
void Sprite::PostDraw(ID3D12GraphicsCommandList* cmdList)
{
	// コマンドリストを解除
...
Sprite* Sprite::Create(UINT texNumber, XMFLOAT2 position, XMFLOAT4 color, XMFLOAT2 anchorpoint)
{
	...
	// Spriteのインスタンスを生成
	Sprite* sprite = new Sprite(texNumber, position, size, color, anchorpoint);
	if (sprite == nullptr) {
		return nullptr;
	}
...
#pragma region 各パラメータを頂点バッファにデータ転送
